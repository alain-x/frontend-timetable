...
const NotificationPanel: React.FC<NotificationPanelProps> = ({ setMessage }) => {
  ...
  const markAsRead = async (notificationId: number) => {
    try {
      ...
      const res = await fetch(`http://localhost:8091/api/notifications/${notificationId}/read`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8091/api/notifications/mark-all-read', {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDismissNotification = async (notificationId: number) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) return;
      const res = await fetch(`http://localhost:8091/api/notifications/${notificationId}/dismiss`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
